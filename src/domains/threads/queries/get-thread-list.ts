...
export function createGetThreadListQuery(deps: GetThreadListQueryDeps) {
  ...
  return function getThreadList(
    ...
    const fetchThreadList = (docId: string) =>
      postGraphqlBatch({
        defaultFuncs,
        ctx,
        form: {
    ...
    fetchThreadList(PRIMARY_THREAD_LIST_DOC_ID)
      .then((resData: Loose) => {
      ...
      .catch((error: Loose) => {
        logError?.("getThreadList primary doc_id failed, trying fallback", error);

        return fetchThreadList(FALLBACK_THREAD_LIST_DOC_ID)
          .then((resData: Loose) => {
            if (!Array.isArray(resData) || resData.length === 0) {
          ...
          .catch((fallbackError: Loose) => {
            logError?.("getThreadList fallback", fallbackError);
            cb(fallbackError);
          });
      });
